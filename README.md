# 学习Redis

- [什么是redis](redis-basic/1-what-is-redis.md)
- [redis的数据类型](redis-basic/2-redis-value-data-type.md)
  * [string字符串类型](redis-basic/4-redis-string.md)
  * [list列表类型](redis-basic/5-redis-list.md)
  * [hash散列](redis-basic/6-redis-hash.md)
  * [set集合](redis-basic/7-redis-set.md)
  * [zset有序集合](redis-basic/8-redis-zset.md)
  * [hyperloglog基数](redis-basic/9-redis-hyperloglog.md)
  * [stream队列](redis-basic/10-redis-stream.md)
- [redis的key键](redis-basic/3-redis-key.md)
- [redis的客户端、服务端](redis-basic/11-redis-client-server.md)
- [redis的Pub/Sub发布订阅模式](redis-basic/12-redis-pub-sub.md)
- [redis的事物](redis-basic/13-redis-transaction.md)
- [redis的持久化](redis-basic/14-redis-rdb-aof.md)
- [redis的分布式锁](redis-basic/15-redis-lock.md)
- [redis的缓存问题](redis-basic/16-redis-cache.md)
- [redisson的redlock算法实现](redis-basic/17-redisson.md)


# 学习 Spring Data Redis

- [springboot集成redis](spring-data-redis/springboot集成redis.md)
- [springboot使用redis stream 实现消息队列](spring-redis-stream/spring-redis-stream.md)

----
## 参考

- [redis教程](http://c.biancheng.net/redis/)
- [Redlock（redis分布式锁）原理分析](https://www.cnblogs.com/rgcLOVEyaya/p/RGC_LOVE_YAYA_1003days.html)
- [分布式锁的实现之redis篇](https://xiaomi-info.github.io/2019/12/17/redis-distributed-lock/)
- [redis分布式锁的实现（setNx命令和Lua脚本）](https://www.cnblogs.com/caibaotimes/p/14419675.html)
- [spring-data-redis](https://spring.io/projects/spring-data-redis)